...
class AuthService:
    ...
    def register(self, user_in: UserCreate) -> User:
        ...
        # 2. Проверяем email (если в репозитории есть такой метод, иначе можно пропустить или добавить try/except)
        # Предполагаем, что в UserRepository есть get_by_email, если нет - закомментируй следующие 4 строки
        ...
        # 4. Подготавливаем данные для создания (исключаем чистый пароль, добавляем хеш)
        user_data = user_in.dict(exclude={"password"})
        user_data["password_hash"] = hashed_password
        
        # 5. Создаем пользователя через репозиторий
        # UserRepository должен иметь метод create. Если его нет, он наследуется от BaseRepository.
