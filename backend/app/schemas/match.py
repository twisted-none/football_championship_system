...
class MatchCreate(MatchBase):
    ...
    def validate_date_not_in_past(cls, v):
        ...
        # Приводим обе даты к одному типу для корректного сравнения
        now = datetime.now()
        
        # Если входящая дата имеет timezone, то и текущую делаем с timezone
        if v.tzinfo is not None:
            now = datetime.now(timezone.utc)
        # Если входящая дата без timezone, убираем timezone у текущей даты
